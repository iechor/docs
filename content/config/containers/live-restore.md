...
keywords: iechor, upgrade, daemon, iechord, live-restore, daemonless container
title: Live restore
aliases:
  - /engine/admin/live-restore/
---

By default, when the iEchor daemon terminates, it shuts down running containers.
...
> Linux containers running on iEchor Desktop for Windows.
...
- Add the configuration to the daemon configuration file. On Linux, this
  defaults to `/etc/iechor/daemon.json`. On iEchor Desktop for Mac or iEchor
  Desktop for Windows, select the iEchor icon from the task bar, then click
  **Settings** -> **iEchor Engine**.
  ...
  - Restart the iEchor daemon. On Linux, you can avoid a restart (and avoid any
    downtime for your containers) by reloading the iEchor daemon. If you use
    `systemd`, then use the command `systemctl reload iechor`. Otherwise, send a
    `SIGHUP` signal to the `iechord` process.

- If you prefer, you can start the `iechord` process manually with the
  ...
  when starting the iEchor process. This can cause unexpected behavior.

## Live restore during upgrades

Live restore allows you to keep containers running across iEchor daemon updates,
...
the iEchor daemon to flush them.

On Linux, you can modify the kernel's buffer size by changing
`/proc/sys/fs/pipe-max-size`. You can't modify the buffer size on iEchor Desktop for
Mac or iEchor Desktop for Windows.
