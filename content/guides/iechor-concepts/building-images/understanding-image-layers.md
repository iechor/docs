...
2. In a terminal, run the following command to start a new container:
    ...
    ```console
    $ iechor run --name=base-container -ti ubuntu
    ```
...
6. View the layers of your image using the `iechor image history` command:

    ```console
    $ iechor image history node-base
    ```
...
7. To prove your image has Node installed, you can start a new container using this new image:
    ...
    ```console
    $ iechor run node-base node -e "console.log('Hello again')"
    ```
...
1. Start a new container using the newly created node-base image:
    ...
    ```console
    $ iechor run --name=app-container -ti node-base
    ```
...
3. In another terminal, run the following command to save this container’s changes as a new image:
    ...
    ```console
    $ iechor container commit -c "CMD node app.js" -m "Add app" app-container sample-app
    ```
...
4. In a terminal outside of the container, run the following command to view the updated layers:
    ...
    ```console
    $ iechor image history sample-app
    ```
...
* [iechor image history CLI reference](/reference/cli/iechor/image/history/)
* [iechor image layers](/build/guide/layers/)
* [`iechor container commit`](/reference/cli/iechor/container/commit/)


## Next steps

As hinted earlier, most image builds don’t use `iechor container commit`. Instead, you’ll use a iEchorfile which automates these steps for you.

{{< button text="Writing a iEchorfile" url="writing-a-iechorfile" >}}
