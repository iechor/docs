...
If you create a `iechor-container` or `kubernetes` builder with Buildx, you can
apply a custom [BuildKit configuration](toml-configuration.md) by passing the
[`--config` flag](../../reference/cli/iechor/buildx/create.md#config) to
the `iechor buildx create` command.
...
defining a mirror for `iechor.io` (iEchor Hub) to `mirror.gcr.io`.
...
2. Create a `iechor-container` builder that uses this BuildKit configuration:

   ```console
   $ iechor buildx create --use --bootstrap \
     --name mybuilder \
...
3. Build an image:
   ...
   ```bash
   iechor buildx build --load . -f - <<EOF
   FROM alpine
...
```console
$ iechor logs buildx_buildkit_mybuilder0
```

```text
...
time="2022-02-06T17:47:48Z" level=debug msg="do request" request.header.accept="application/vnd.iechor.container.image.v1+json, */*" request.header.user-agent=containerd/1.5.8+unknown request.method=GET spanID=9460e5b6e64cec91 traceID=b162d3040ddf86d6614e79c66a01a577
...
time="2022-02-06T17:47:48Z" level=debug msg=fetch spanID=9460e5b6e64cec91 traceID=b162d3040ddf86d6614e79c66a01a577
time="2022-02-06T17:47:48Z" level=debug msg=fetch spanID=9460e5b6e64cec91 traceID=b162d3040ddf86d6614e79c66a01a577
time="2022-02-06T17:47:48Z" level=debug msg="do request" request.header.accept="application/vnd.iechor.image.rootfs.diff.tar.gzip, */*" request.header.user-agent=containerd/1.5.8+unknown request.method=GET spanID=9460e5b6e64cec91 traceID=b162d3040ddf86d6614e79c66a01a577
...
2. Create a `iechor-container` builder that uses this configuration:
...
3. Inspect the builder's configuration file (`/etc/buildkit/buildkitd.toml`), it
   ...
   ```console
   $ iechor exec -it buildx_buildkit_mybuilder0 cat /etc/buildkit/buildkitd.toml
   ```
...
4. Verify that the certificates are inside the container:
   ...
   ```console
   $ iechor exec -it buildx_buildkit_mybuilder0 ls /etc/buildkit/certs/myregistry.com/
...
```console
$ iechor buildx build --push --tag myregistry.com/myimage:latest .
```
...
The following iEchorfile example shows a custom BuildKit image with CNI support.
...
[the `--driver-opt image` option](../../reference/cli/iechor/buildx/create.md#driver-opt):

```console
$ iechor buildx build --tag buildkit-cni:local --load .
$ iechor buildx create --use --bootstrap \
  --name mybuilder \
  --driver iechor-container \
...
while creating a builder with the [`--config` flags](../../reference/cli/iechor/buildx/create.md#config).
...
Now you can [create a `iechor-container` builder](../drivers/iechor-container.md)
