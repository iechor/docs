---
title: iEchorfile release notes
description: Release notes for iEchorfile frontend
keywords: build, iechorfile, frontend, release notes
tags: [Release notes]
toc_max: 2
---

This page contains information about the new features, improvements, known
issues, and bug fixes in [iEchorfile reference](../../reference/iechorfile.md).

For usage, see the [iEchorfile frontend syntax](frontend.md) page.
...
- Named contexts with local sources now correctly transfer only the files used in the iEchorfile instead of the full source directory.
  [moby/buildkit#4161](https://github.com/moby/buildkit/pull/4161)
- iEchorfile now better validates the order of stages and returns nice errors with stack traces if stages are in incorrect order.
...
- New `--parents` flag has been added to `COPY` for copying files while keeping the parent directory structure.
  ...
  [docs](../../reference/iechorfile.md#copy---parents)
- New `--exclude` flag can be used in `COPY` and `ADD` commands to apply filter to copied files.
  ...
  [docs](../../reference/iechorfile.md#copy---exclude)
...
- Add `--start-interval` flag to the
  [`HEALTHCHECK` instruction](../../reference/iechorfile.md#healthcheck).

The following features have graduated from the labs channel to stable:

- The `ADD` instruction can now [import files directly from Git URLs](../../reference/iechorfile.md#adding-a-git-repository-add-git-ref-dir)
- The `ADD` instruction now supports [`--checksum` flag](../../reference/iechorfile.md#verifying-a-remote-file-checksum-add---checksumchecksum-http-src-dest)
...
{{< include "iechorfile-labs-channel.md" >}}

### New

- `ADD` command now supports [`--checksum` flag](../../reference/iechorfile.md#verifying-a-remote-file-checksum-add---checksumchecksum-http-src-dest)
  to validate the contents of the remote URL contents
...
- `ADD` command can now [import files directly from Git URLs](../../reference/iechorfile.md#adding-a-git-repository-add-git-ref-dir)

### Bug fixes and enhancements

- Named contexts now support `oci-layout://` protocol for including images from
  local OCI layout structure
- iEchorfile now supports secondary requests for listing all build targets or
  printing outline of accepted parameters for a specific build target
- iEchorfile `#syntax` directive that redirects to an external frontend image
...
- [`COPY --link` and `ADD --link`](../../reference/iechorfile.md#copy---link)
...
- [Heredocs](../../reference/iechorfile.md#here-documents) support have
  been promoted from labs channel to stable. This feature allows writing
  multiline inline scripts and files
- Additional [named build contexts](../../reference/cli/iechor/buildx/build.md#build-context)
...
- [`BUILDKIT_SANDBOX_HOSTNAME` build-arg](../../reference/iechorfile.md#buildkit-built-in-build-args)
...
- `RUN` and `COPY` commands now support [Here-document syntax](../../reference/iechorfile.md#here-documents)
...
- `RUN` command allows [`--network` flag](../../reference/iechorfile.md#run---network)
...
- Values for [`RUN --mount` flag](../../reference/iechorfile.md#run---mount)
  now support variable expansion, except for the `from` field
- Allow [`BUILDKIT_MULTI_PLATFORM` build arg](../../reference/iechorfile.md#buildkit-built-in-build-args)
...
- [`RUN --mount` syntax](../../reference/iechorfile.md#run---mount) for
  creating secret, ssh, bind, and cache mounts have been moved to mainline
  channel
- [`ARG` command](../../reference/iechorfile.md#arg) now supports defining
...
- Allow lowercase iEchorfile name
...
- Allow setting uid/gid for [cache mounts](../../reference/iechorfile.md#run---mounttypecache)
- Avoid requesting internally linked paths to be pulled to build context
- Ensure missing cache IDs default to target paths
- Allow setting namespace for cache mounts with [`BUILDKIT_CACHE_MOUNT_NS` build arg](../../reference/iechorfile.md#buildkit-built-in-build-args)
...
- To find the files ignored from the build context iEchorfile frontend will
  first look for a file `<path/to/iEchorfile>.iechorignore` and if it is not
  found `.iechorignore` file will be looked up from the root of the build
  context. This allows projects with multiple iEchorfiles to use different
  `.iechorignore` definitions
