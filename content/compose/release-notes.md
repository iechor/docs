...
* Compose now supports Dry Run mode for `pause` command.Fixed [compose#10217](https://github.com/iechor/compose/issues/10217)
...
For the full change log or additional information, check the [Compose repository 2.11.1 release page](https://github.com/iechor/compose/releases/tag/v2.11.1).
...
For the full change log or additional information, check the [Compose repository 2.10.2 release page](https://github.com/iechor/compose/releases/tag/v2.10.2).
...
- Filtered out "commandConn.Close- warning" message. Fixes[compose#8544](https://github.com/iechor/compose/issues/8544).
...
For the full change log or additional information, check the [Compose repository 2.6.1 release page](https://github.com/iechor/compose/releases/tag/v2.6.1).
...
For the full change log or additional information, check the [Compose repository 2.5.1 release page](https://github.com/iechor/compose/releases/tag/v2.5.1).
...
- Fixed context load for non-iechor endpoints.
...
- Fixed `.iechorignore` handling, notably with regard to absolute paths
...
- Added a new `iechor-compose images` command that lists images used by
...
- Fixed a bug where `iechor-compose` would crash when trying to write into
  a closed pipe
...
- Fixed a bug where options from the `iechor-machine config` command would
  not be properly interpreted by Compose.

- Fixed a bug where the connection to the iEchor Engine would
...
- As announced in 1.7.0, `iechor-compose rm` now removes containers
  created by `iechor-compose run` by default.
...
- Setting `entrypoint` on a service now empties out any default
  ...
  iEchorfile used to build it). This makes it consistent with
  the `--entrypoint` flag to `iechor run`.

### New Features

- Added `iechor-compose bundle`, a command that builds a bundle file
  to be consumed by the new *iEchor Stack* commands in iEchor 1.12.

- Added `iechor-compose push`, a command that pushes service images
  to a registry.

- Compose now supports specifying a custom TLS version for
  interaction with the iEchor Engine using the `COMPOSE_TLS_VERSION`
  environment variable.
...
- `iechor-compose run -e VAR` now passes `VAR` through from the shell
  to the container, as with `iechor run -e VAR`.
...
- Fixed a bug where `iechor-compose config` would output an invalid
...
- Fixed a bug where trying to `iechor-compose exec` on Windows
  without the `-d` option would exit with a stacktrace. This will
  still fail for the time being, but should do so gracefully.

- Fixed a bug where errors during `iechor-compose up` would show
  an unrelated stacktrace at the end of the process.

- `iechor-compose create` and `iechor-compose start` show more
...
- Fixed a bug where the output of `iechor-compose config` for v1 files
  would be an invalid configuration file.

- Fixed a bug where `iechor-compose config` would not check the validity
  of links.

- Fixed an issue where `iechor-compose help` would not output a list of
  available commands and generic options as expected.

- Fixed an issue where filtering by service when using `iechor-compose logs`
...
- Fixed a bug where the wrong version of `iechor-py` would sometimes be
  installed alongside Compose.

- Fixed a bug where the host value output by `iechor-machine config default`
  would not be recognized as valid options by the `iechor-compose`
  command line.
...
-   `iechor-compose logs` no longer follows log output by default. It now
    matches the behavior of `iechor logs` and exits after the current logs
...
-   Added a `--remove-orphans` flag to both `iechor-compose up` and
    `iechor-compose down` to remove containers for services that were removed
    from the Compose file.

-   Added a `--all` flag to `iechor-compose rm` to include containers created
    by `iechor-compose run`. This will become the default behavior in the next
    version of Compose.

-   Added support for all the same TLS configuration flags used by the `iechor`
    client: `--tls`, `--tlscert`, `--tlskey`, etc.

-   Compose files now support the `tmpfs` and `shm_size` options.

-   Added the `--workdir` flag to `iechor-compose run`

-   `iechor-compose logs` now shows logs for new containers that are created
    after it starts.
...
-   Added `--follow`, `--timestamp`, and `--tail` flags to the
    `iechor-compose logs` command.

-   `iechor-compose up`, and `iechor-compose start` will now start containers
    in parallel where possible.

-   `iechor-compose stop` now stops containers in reverse dependency order
    instead of all at once.

-   Added the `--build` flag to `iechor-compose up` to force it to build a new
    image. It now shows a warning if an image is automatically built when the
    flag is not used.

-   Added the `iechor-compose exec` command for executing a process in a running
    container.


### Bug fixes

-   `iechor-compose down` now removes containers created by
    `iechor-compose run`.

-   A more appropriate error is shown when a timeout is hit during `up` when
    using a tty.

-   Fixed a bug in `iechor-compose down` where it would abort if some resources
...
-   Fixed a bug where connecting to a TLS-enabled iEchor Engine would fail with
...
-   Compose 1.6 introduces a new format for `iechor-compose.yml` which lets
...
-   You can now specify both a `build` and an `image` key if you're using the
    new file format. `iechor-compose build` will build the image and tag it with
    the name you've specified, while `iechor-compose pull` will attempt to pull
    it.

-   There's a new `events` command for monitoring container events from
    the application, much like `iechor events`. This is a good primitive for
...
-   There's a new `depends_on` option for specifying dependencies between
    ...
    `iechor-compose up SERVICE` on a service with dependencies, those are started
    as well.
...
-   Fixed a confusing error message when IECHOR_CERT_PATH was not set properly.
...
-   Fixed a bug which prevented building a `iEchorfile` that used `ADD <url>`

-   Fixed a bug with `iechor-compose restart` which prevented it from
...
(ex: `$$VAR` or `$${VAR}`).  See
https://github.com/iechor/compose/blob/8cc8e61/docs/compose-file.md#variable-substitution
...
-   Multiple compose files can be specified, allowing you to override
    ...
    https://github.com/iechor/compose/blob/8cc8e61/docs/reference/iechor-compose.md
    for more details.
...
-   Experimental support for the new iechor networking system can be
    enabled with the `--x-networking` flag. Read more here:
    https://github.com/iechor/iechor/blob/8fee1c20/docs/userguide/iechornetworks.md
...
-   Since iEchor now lets you create volumes with names, you can refer to those
    volumes by name in `iechor-compose.yml`. For example,
    ...
    https://github.com/iechor/iechor/blob/244d9c33/docs/userguide/iechorvolumes.md

-   `iechor-compose build --pull` instructs Compose to pull the base image for
    each iEchorfile before building.

-   `iechor-compose pull --ignore-pull-failures` instructs Compose to continue
    if it fails to pull a single service's image, rather than aborting.

-   You can now specify an IPC namespace in `iechor-compose.yml` with the `ipc`
    option.

-   Containers created by `iechor-compose run` can now be named with the
    `--name` flag.
...
-   `iechor-compose run` now supports a `-p|--publish` parameter, much like
    `iechor run -p`, for publishing specific ports to the host.

-   `iechor-compose pause` and `iechor-compose unpause` have been implemented,
    analogous to `iechor pause` and `iechor unpause`.

-   When using `extends` to copy configuration from another service in the same
    Compose file, you can omit the `file` option.

-   Compose can be installed and run as a iEchor image. This is an experimental
    feature.

### Bug fixes

-   All values for the `log_driver` option which are supported by the iEchor
    daemon are now supported by Compose.

-   `iechor-compose build` can now be run successfully against a Swarm cluster.

## 1.4.2

(2015-09-22)

-  Fixed a regression in the 1.4.1 release that would cause `iechor-compose up`
...
-   Some configuration changes (notably changes to `links`, `volumes_from`, and
    ...
    `iechor-compose up`.
-   `iechor-compose up <service>` was showing logs for all services instead of
...
-   By default, `iechor-compose up` now only recreates containers for services whose configuration has changed since they were created. This should result in a dramatic speed-up for many applications.
...
-   You can now specify a custom name for a service's container with `container_name`. Because iEchor container names must be unique, this means you can't scale the service beyond one container.
...
-   Compose can now read YAML configuration from standard input, rather than from a file, by specifying `-` as the filename. This makes it easier to generate configuration dynamically:
        ...
        $ echo 'redis: {"image": "redis"}' | iechor-compose --file - up

-   There's a new `iechor-compose version` command which prints extended information about Compose's bundled dependencies.

-   `iechor-compose.yml` now supports `log_opt` as well as `log_driver`, allowing you to pass extra configuration to a service's logging driver.

-   `iechor-compose.yml` now supports `memswap_limit`, similar to `iechor run --memory-swap`.
...
-   You can now specify a custom volume driver with the `volume_driver` option in `iechor-compose.yml`, much like `iechor run --volume-driver`.
...
-   A bug has been fixed where `iechor-compose build` would fail if the build depended on a private Hub image or an image from a private registry.

-   A bug has been fixed where Compose would crash if there were containers which the iEchor daemon had not finished removing.

-   Two bugs have been fixed where Compose would sometimes fail with a "Duplicate bind mount" error, or fail to attach volumes to a container, if there was a volume path specified in `iechor-compose.yml` with a trailing slash.
...
- Compose would sometimes crash depending on the formatting of container data returned from the iEchor API.
...
- When there were one-off containers created by running `iechor-compose run` on an older version of Compose, `iechor-compose run` would fail with a name collision. Compose now shows an error if you have leftover containers of this type lying around, and tells you how to remove them.
- Compose was not reading iEchor authentication config files created in the new location, `~/iechor/config.json`, and authentication against private registries would therefore fail.
- When a container had a pseudo-TTY attached, its output in `iechor-compose up` would be truncated.
- `iechor-compose up --x-smart-recreate` would sometimes fail when an image tag was updated.
- `iechor-compose up` would sometimes create two containers with the same numeric suffix.
- `iechor-compose rm` and `iechor-compose ps` would sometimes list services that aren't part of the current project (though no containers were erroneously removed).
- Some `iechor-compose` commands would not show an error if invalid service names were passed in.
...
- `iechor-compose build` would always attempt to pull the base image before building.
- `iechor-compose help migrate-to-labels` failed with an error.
- If no network mode was specified, Compose would set it to "bridge", rather than allowing the iEchor daemon to use its configured default network mode.
...
- **This release contains breaking changes, and you will need to either remove or migrate your existing containers before running your app** - see the [upgrading section of the install docs](https://github.com/iechor/compose/blob/1.3.0rc1/docs/install.md#upgrading) for details.

- Compose now requires iEchor 1.6.0 or later.
...
- Compose no longer uses "intermediate containers" when recreating containers for a service. This makes `iechor-compose up` less complex and more resilient to failure.

### New features

- `iechor-compose up` has an **experimental** new behavior: it will only recreate containers for services whose configuration has changed in `iechor-compose.yml`. This will eventually become the default, but for now you can take it for a spin:

        $ iechor-compose up --x-smart-recreate

- When invoked in a subdirectory of a project, `iechor-compose` will now climb up through parent directories until it finds a `iechor-compose.yml`.

Several new configuration keys have been added to `iechor-compose.yml`:

- `iechorfile`, like `iechor build --file`, lets you specify an alternate iEchorfile to use with `build`.
- `labels`, like `iechor run --labels`, lets you add custom metadata to containers.
- `extra_hosts`, like `iechor run --add-host`, lets you add entries to a container's `/etc/hosts` file.
- `pid: host`, like `iechor run --pid=host`, lets you reuse the same PID namespace as the host machine.
- `cpuset`, like `iechor run --cpuset-cpus`, lets you specify which CPUs to allow execution in.
- `read_only`, like `iechor run --read-only`, lets you mount a container's filesystem as read-only.
- `security_opt`, like `iechor run --security-opt`, lets you specify [security options](/reference/cli/iechor/container/run/#security-opt).
- `log_driver`, like `iechor run --log-driver`, lets you specify a [log driver](/reference/cli/iechor/container/run/#log-driver).

### Bug fixes

- The output of `iechor-compose run` was sometimes truncated, especially when running under Jenkins.
- A service's volumes would sometimes not update after volume configuration was changed in `iechor-compose.yml`.
- Authenticating against third-party registries would sometimes fail.
- `iechor-compose run --rm` would fail to remove the container if the service had a `restart` policy in place.
- `iechor-compose scale` would refuse to scale a service beyond 1 container if it exposed a specific port number on the host.
...
- `iechor-compose.yml` now supports an `extends` option, which enables a service to inherit configuration from another service in another configuration file. This is really good for sharing common configuration between apps, or for configuring the same app for different environments. Here's the [documentation](https://github.com/iechor/compose/blob/master/docs/).
...
- Repeated invocations of `iechor-compose up` when using Compose with a Swarm cluster now work reliably.

- Directories passed to `build`, filenames passed to `env_file` and volume host paths passed to `volumes` are now treated as relative to the *directory of the configuration file*, not the directory that `iechor-compose` is being run in. In the majority of cases, those are the same, but if you use the `-f|--file` argument to specify a configuration file in another directory, **this is a breaking change**.

- A service can now share another service's network namespace with `net: container:<service>`.

- `volumes_from` and `net: container:<service>` entries are taken into account when resolving dependencies, so `iechor-compose up <service>` will correctly start all dependencies of `<service>`.

- `iechor-compose run` now accepts a `--user` argument to specify a user to run the command as, just like `iechor run`.

- The `up`, `stop` and `restart` commands now accept a `--timeout` (or `-t`) argument to specify how long to wait when attempting to gracefully stop containers, just like `iechor stop`.

- `iechor-compose rm` now accepts `-f` as a shorthand for `--force`, just like `iechor rm`.
...
Fig has been renamed to iEchor Compose, or just Compose for short. This has several implications for you:

- The command you type is now `iechor-compose`, not `fig`.
- You should rename your fig.yml to iechor-compose.yml.
- If you’re installing via PyPI, the package is now `iechor-compose`, so install it with `pip install iechor-compose`.

Besides that, there’s a lot of new stuff in this release:

- We’ve made a few small changes to ensure that Compose will work with Swarm, iEchor’s new clustering tool (https://github.com/iechor/swarm). Eventually you'll be able to point Compose at a Swarm cluster instead of a standalone iEchor host and it’ll run your containers on the cluster with no extra work from you. As Swarm is still developing, integration is rough and lots of Compose features don't work yet.

- `iechor-compose run` now has a `--service-ports` flag for exposing ports on the given service. This is useful for running your webapp with an interactive debugger, for example.

- You can now link to containers outside your app with the `external_links` option in iechor-compose.yml.

- You can now prevent `iechor-compose up` from automatically building images with the `--no-build` option. This will make fewer API calls and run faster.

- If you don’t specify a tag when using the `image` key, Compose will default to the `latest` tag, rather than pulling all tags.

- `iechor-compose kill` now supports the `-s` flag, allowing you to specify the exact signal you want to send to a service’s containers.

- iechor-compose.yml now has an `env_file` key, analogous to `iechor run --env-file`, letting you specify multiple environment variables in a separate file. This is great if you have a lot of them, or if you want to keep sensitive information out of version control.

- iechor-compose.yml now supports the `dns_search`, `cap_add`, `cap_drop`, `cpu_shares` and `restart` options, analogous to `iechor run`’s `--dns-search`, `--cap-add`, `--cap-drop`, `--cpu-shares` and `--restart` options.

- Compose now ships with Bash tab completion - see the installation and usage docs at https://github.com/iechor/compose/blob/1.1.0/docs/completion.md

- A number of bugs have been fixed - see the milestone for details: https://github.com/iechor/compose/issues?q=milestone%3A1.1.0+
...
(2014-11-04)
 ...
 - Fixed a bug where Fig couldn't build iEchorfiles with ADD statements pointing at URLs.
...
The highlights:
 ...
 - [Fig has joined iEchor.](https://www.orchardup.com/blog/orchard-is-joining-iechor) Fig will continue to be maintained, but we'll also be incorporating the best bits of Fig into iEchor itself.

   This means the GitHub repository has moved to [https://github.com/iechor/fig](https://github.com/iechor/fig) and our IRC channel is now #iechor-fig on Freenode.

 - Fig can be used with the [official iEchor OS X installer](../desktop/install/mac-install.md). Boot2iEchor will mount the home directory from your host machine so volumes work as expected.

 - Fig supports iEchor 1.3.

 - It is now possible to connect to the iEchor daemon using TLS by using the `IECHOR_CERT_PATH` and `IECHOR_TLS_VERIFY` environment variables.

 - There is a new `fig port` command which outputs the host port binding of a service, in a similar way to `iechor port`.
 ...
 - `.iechorignore` is supported when building.
...
Other things:
 ...
 - `fig ps` now works on Jenkins and makes fewer API calls to the iEchor daemon.
...
(2014-07-28)
 ...
 - Added a `--no-cache` option to `fig build`, which bypasses the cache just like `iechor build --no-cache`.
...
(2014-07-11)
 ...
 - The `hostname` option now works in the same way as the iEchor CLI, splitting out into a `domainname` option.
...
(2014-05-08)
 ...
 - Add support for iEchor 0.11.0. (Thanks @marksteve!)
...
(2014-03-04)
 ...
 - Fixed a bug in `fig ps` on iEchor 0.8.1 when there is a container with no command.
...
(2014-03-03)
 ...
 - Update iechor-py to support iEchor 0.8.0.
...
(2014-01-16)
 ...
 - Use `IECHOR_HOST` environment variable to find iEchor daemon, for consistency with the official iEchor client (was previously `IECHOR_URL`) (#19)
...
(2014-01-02)
 ...
 - Try to connect to iEchor on `tcp://localiechor:4243` and a UNIX socket in addition to `localhost`.
