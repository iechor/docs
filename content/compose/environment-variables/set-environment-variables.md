...
An `.env` file in iEchor Compose is a text file used to define environment variables that should be made available to iEchor containers when running `iechor compose up`. This file typically contains key-value pairs of environment variables, and it allows you to centralize and manage configuration in one place. The `.env` file is useful if you have multiple environment variables you need to store.
...
When you run `iechor compose up`, the `web` service defined in the Compose file [interpolates](env-file.md#interpolation) in the
image `webapp:v1.5` which was set in the `.env` file. You can verify this with the
[config command](../../reference/cli/iechor/compose/config.md), which prints your resolved application config to the terminal:
...
- As of iEchor Compose version 2.24.0, you can set your `.env` file to be optional by using the `env_file` attribute. When `required` is set to `false` and the `.env` file is missing, Compose silently ignores the entry.
  ```yaml
  env_file:
    - path: ./default.env
    ...
    services:
      webapp:
        ...
        environment:
          - DEBUG=${DEBUG}
  ```
  iEchor Compose replaces `${DEBUG}` with the value from the `.env` file
- You can use multiple `.env` files in your `compose.yml` with the [`env_file` attribute](../compose-file/05-services.md#env_file), and iEchor Compose reads them in the order specified. If the same variable is defined in multiple files, the last definition takes precedence:
  ```yaml
  services:
    webapp:
      image: my-webapp-image
...
- Values in your `.env` file can be overridden from the command line by using [`iechor compose run -e`](#set-environment-variables-with-iechor-compose-run---env).
- Your `.env` file can be overridden by another `.env` if it is [substituted with `--env-file`](#substitute-with---env-file).

> **Important**
>
> Substitution from `.env` files is a iEchor Compose CLI feature.
>
> It is not supported by Swarm when running `iechor stack deploy`.
{ .important }

### Use the `environment` attribute

You can set environment variables directly in your Compose file without using an `.env` file, with the
[`environment` attribute](../compose-file/05-services.md#environment) in your `compose.yml`. It works in the same way as `iechor run -e VARIABLE=VALUE ...`
...
- You can choose not to set a value and pass the environment variables from your shell straight through to your containers. It works in the same way as `iechor run -e VARIABLE ...`:
  ```yaml
  web:
    environment:
      - DEBUG
...
It works in the same way as `iechor run --env-file=FILE ...`.
...
- Values in your `.env` files can be overridden from the command line by using [`iechor compose run -e`](#set-environment-variables-with-iechor-compose-run---env).
- Your `.env` files can be overriden by another `.env` if it is [substituted with `--env-file`](#substitute-with---env-file).
- As of iEchor Compose version 2.24.0, you can set your `.env` file to be optional by using the `required` field. When `required` is set to `false` and the `.env` file is missing,
...
You can use existing environment variables from your host machine or from the shell environment where you execute `iechor compose` commands. This allows you to dynamically inject values into your iEchor Compose configuration at runtime.
...
When you run `iechor compose up` with this configuration, Compose looks for the `POSTGRES_VERSION` environment variable in the shell and substitutes its value in. For this example, Compose resolves the image to `postgres:9.3` before running the configuration.

If an environment variable is not set, Compose substitutes with an empty string. In the example above, if `POSTGRES_VERSION` is not set, the value for the image option is `postgres:`.

> **Note**
>
> `postgres:` is not a valid image reference. iEchor expects either a reference without a tag, like `postgres` which defaults to the latest image, or with a tag such as `postgres:15`.
...
The advantage of this method is that you can store the file anywhere and name it appropriately, for example, 
 ...
 This file path is relative to the current working directory where the iEchor Compose command is executed. Passing the file path is done using the `--env-file` option:

```console
$ iechor compose --env-file ./config/.env.dev up
```
...
- This method is useful if you want to temporarily override an `.env` file that is already referenced in your `compose.yml` file. For example you may have different `.env` files for production ( `.env.prod`) and testing (`.env.test`).
  ...
  ```console
  $ iechor compose --env-file ./config/.env.dev config
  services:
    web:
      image: 'webapp:v1.6'
  ```
  ...
  ```console
  $ iechor compose --env-file ./doesnotexist/.env.dev  config
  ERROR: Couldn't find env file: /home/user/./doesnotexist/.env.dev
  ```

- You can use multiple `--env-file` options to specify multiple environment files, and iEchor Compose reads them in order. Later files can override variables from earlier files.
  ```console
  $ iechor compose --env-file .env --env-file .env.override up
  ```
- You can override specific environment variables from the command line when starting containers. 
  ```console
  $ iechor compose --env-file .env.dev up -e DATABASE_URL=mysql://new_user:new_password@new_db:3306/new_database
  ```

### Set environment variables with `iechor compose run --env`

Similar to `iechor run --env`, you can set environment variables temporarily with `iechor compose run --env` or its short form `iechor compose run -e`:

```console
$ iechor compose run -e DEBUG=1 web python console.py
```
...
- You can also pass a variable from the shell by not giving it a value:
  ...
  ```console
  $ iechor compose run -e DEBUG web python console.py
  ```
