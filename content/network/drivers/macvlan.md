...
case, you need to designate a physical interface on your iEchor host to use for
...
- If your application can work using a bridge (on a single iEchor host) or
  overlay (to communicate across multiple iEchor hosts), these solutions may be
...
- In 802.1Q trunk bridge mode, traffic goes through an 802.1Q sub-interface
  which iEchor creates on the fly. This allows you to control routing and
  filtering at a more granular level.

### Bridge mode

To create a `macvlan` network which bridges with a given physical network
interface, use `--driver macvlan` with the `iechor network create` command. You
also need to specify the `parent`, which is the interface the traffic will
physically go through on the iEchor host.
...
iEchor interprets that as a sub-interface of `eth0` and creates the sub-interface
automatically.
...
If you have [configured the iEchor daemon to allow IPv6](../../config/daemon/ipv6.md),
