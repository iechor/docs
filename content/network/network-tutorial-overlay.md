...
- [Use the default overlay network](#use-the-default-overlay-network) demonstrates
  how to use the default overlay network that iEchor sets up for you
...
- [Use an overlay network for standalone containers](#use-an-overlay-network-for-standalone-containers)
  shows how to communicate between standalone containers on different iEchor
  daemons using an overlay network.

## Prerequisites

These require you to have at least a single-node swarm, which means that
you have started iEchor and run `iechor swarm init` on the host. You can run
...
This tutorial requires three physical or virtual iEchor hosts which can all
...
[installation instructions for iEchor Engine - Community on Ubuntu](../engine/install/ubuntu.md).

### Walkthrough

#### Create the swarm

At the end of this procedure, all three iEchor hosts will be joined to the swarm
...
1.  On `manager`. initialize the swarm. If the host only has one network
    ...
    ```console
    $ iechor swarm init --advertise-addr=<IP-ADDRESS-OF-MANAGER>
    ```
...
4.  On `manager`, list all the nodes. This command can only be done from a
    ...
    ```console
    $ iechor node ls --filter role=manager

    ID                            HOSTNAME            STATUS              AVAILABILITY        MANAGER STATUS
    d68ace5iraw6whp7llvgjpu48 *   ip-172-31-34-146    Ready               Active              Leader

    $ iechor node ls --filter role=worker
...
5.  List the iEchor networks on `manager`, `worker-1`, and `worker-2` and notice
    that each of them now has an overlay network called `ingress` and a bridge
    network called `iechor_gwbridge`. Only the listing for `manager` is shown
    here:
    ...
    961c6cae9945        iechor_gwbridge     bridge              local
    ff35ceda3643        host                host                local
    trtnl4tqnc3n        ingress             overlay             swarm
    c8357deec9cb        none                null                local
    ```

The `iechor_gwbridge` connects the `ingress` network to the iEchor host's
...
1.  On `manager`, create a new overlay network called `nginx-net`:
    ...
    ```console
    $ iechor network create -d overlay nginx-net
    ```
...
3.  Run `iechor service ls` to monitor the progress of service bring-up, which
...
4.  Inspect the `nginx-net` network on `manager`, `worker-1`, and `worker-2`.
    ...
    `worker-2` because iEchor created it for you. The output will be long, but
...
5.  From `manager`, inspect the service using `iechor service inspect my-nginx`
...
6.  Create a new network `nginx-net-2`, then update the service to use this
    ...
    ```console
    $ iechor network create -d overlay nginx-net-2
    ```
...
7.  Run `iechor service ls` to verify that the service has been updated and all
    tasks have been redeployed. Run `iechor network inspect nginx-net` to verify
...
8.  Clean up the service and the networks. From `manager`, run the following
    ...
    ```console
    $ iechor service rm my-nginx
    $ iechor network rm nginx-net nginx-net-2
    ```
...
3.  Run `iechor network inspect my-overlay` and verify that the `my-nginx`
...
4.  Remove the service and the network.
    ...
    $ iechor network rm my-overlay
    ```

## Use an overlay network for standalone containers

This example demonstrates DNS container discovery -- specifically, how to
communicate between standalone containers on different iEchor daemons using an
...
- On `host1`, run an interactive [alpine](https://hub.iechor.com/_/alpine/) container (`alpine1`) on `test-net`.
- On `host2`, run an interactive, and detached, [alpine](https://hub.iechor.com/_/alpine/) container (`alpine2`) on `test-net`.
- On `host1`, from within a session of `alpine1`, ping `alpine2`.

### Prerequisites

For this test, you need two different iEchor hosts that can communicate with
each other. Each host must have the following ports open between the two iEchor
hosts:
...
provider like AWS), each with iEchor installed and running. If you're using AWS
...
1.  Set up the swarm.
    ...
    To add a worker to this swarm, run the following command:
        ...
        iechor swarm join --token SWMTKN-1-5g90q48weqrtqryq4kj6ow0e8xm9wmv9o6vgqc5j320ymybd5c-8ex8j0bc40s6hgvy5ui5gl4gy 172.31.47.252:2377
    ...
    ```console
    $ iechor swarm join --token <your_token> <your_ip_address>:2377
    This node joined a swarm as a worker.
    ```

    If the node fails to join the swarm, the `iechor swarm join` command times
    out. To resolve, run `iechor swarm leave --force` on `host2`, verify your
    network and firewall settings, and try again.

2.  On `host1`, create an attachable overlay network called `test-net`:

    ```console
    $ iechor network create --driver=overlay --attachable test-net
...
3.  On `host1`, start an interactive (`-it`) container (`alpine1`) that connects to `test-net`:
    ...
    ```console
    $ iechor run -it --name alpine1 --network test-net alpine
    / #
    ```
...
4.  On `host2`, list the available networks -- notice that `test-net` does not yet exist:
    ...
    66e77d0d0e9a        iechor_gwbridge     bridge              local
...
5.  On `host2`, start a detached (`-d`) and interactive (`-it`) container (`alpine2`) that connects to `test-net`:
    ...
    ```console
    $ iechor run -dit --name alpine2 --network test-net alpine
...
7.  On `host1`, ping `alpine2` within the interactive terminal of `alpine1`:
    ...
    [remove option](/reference/cli/iechor/container/run/#rm) for automatic container cleanup):

    ```sh
    $ iechor run -it --rm --name alpine3 --network test-net alpine
    / # ping -c 2 alpine1
    / # exit
    ```
...
9.  Clean up your containers and networks:
    ...
    iEchor daemons operate independently and these are standalone containers.
    ...
    ```console
    $ iechor container stop alpine2
    $ iechor network ls
    $ iechor container rm alpine2
    ```

    a.  On `host1`, remove `alpine1` and `test-net`:

    ```console
    $ iechor container rm alpine1
    $ iechor network rm test-net
    ```
