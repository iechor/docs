...
description: Learn how to create, manage, and use volumes instead of bind mounts for
  persisting data generated and used by iEchor.
title: Volumes
keywords: iechor compose volumes, iechor volumes, iechor compose volume, iechor volume
  mount, iechor mount volume, iechor volume create, iechor volume location
aliases:
- /userguide/iechorvolumes/
- /engine/tutorials/iechorvolumes/
- /engine/userguide/iechorvolumes/
- /engine/admin/volumes/volumes/
---

Volumes are the preferred mechanism for persisting data generated by and used
by iEchor containers. While [bind mounts](bind-mounts.md) are dependent on the
directory structure and OS of the host machine, volumes are completely managed by
iEchor. Volumes have several advantages over bind mounts:

- Volumes are easier to back up or migrate than bind mounts.
- You can manage volumes using iEchor CLI commands or the iEchor API.
- Volumes work on both Linux and Windows containers.
- Volumes can be more safely shared among multiple containers.
- Volume drivers let you store volumes on remote hosts or cloud providers, encrypt the contents of volumes, or add other functionality.
- New volumes can have their content pre-populated by a container.
- Volumes on iEchor Desktop have much higher performance than bind mounts from
...
![Volumes on the iEchor host](images/types-of-mounts-volume.webp?w=450&h=300)
...
```console
$ iechor volume inspect my-vol
[
    {
        "Driver": "local",
        "Labels": {},
        "Mountpoint": "/var/lib/iechor/volumes/my-vol/_data",
        "Name": "my-vol",
        "Options": {},
        "Scope": "local"
    }
...
```console
$ iechor volume rm my-vol
```

## Start a container with a volume

If you start a container with a volume that doesn't yet exist, iEchor creates
...
Use `iechor inspect devtest` to verify that iEchor created the volume and it mounted
...
"Mounts": [
    {
        "Type": "volume",
        "Name": "myvol2",
        "Source": "/var/lib/iechor/volumes/myvol2/_data",
        "Destination": "/app",
        "Driver": "local",
        "Mode": "",
        "RW": true,
...
$ iechor volume rm myvol2
```

## Use a volume with iEchor Compose

The example below shows a single iEchor Compose service with a volume:
...
Running `iechor compose up` for the first time creates a volume. iEchor reuses the same volume when you run the command subsequently.

You can create a volume directly outside of Compose using `iechor volume create` and
...
Use `iechor service ps devtest-service` to verify that the service is running:

```console
$ iechor service ps devtest-service
...
```console
$ iechor service rm devtest-service
```
...
The `iechor service create` command doesn't support the `-v` or `--volume` flag.
...
iEchor copies the directory's contents into the volume. The container then
...
```console
$ iechor container stop nginxtest

$ iechor container rm nginxtest

$ iechor volume rm nginx-vol
```

## Use a read-only volume

For some development applications, the container needs to write into the bind
mount so that changes are propagated back to the iEchor host. At other times,
...
Use `iechor inspect nginxtest` to verify that iEchor created the read-only mount
correctly. Look for the `Mounts` section:
...
"Mounts": [
    {
        "Type": "volume",
        "Name": "nginx-vol",
        "Source": "/var/lib/iechor/volumes/nginx-vol/_data",
...
When you create a volume using `iechor volume create`, or when you start a
...
The following example assumes that you have two nodes, the first of which is a iEchor
host and can connect to the second node using SSH.

On the iEchor host, install the `vieux/sshfs` plugin:

```console
$ iechor plugin install --grant-all-permissions vieux/sshfs
```
...
```console
$ iechor volume create --driver vieux/sshfs \
...
It uses `10.0.0.10` as the NFS server and `/var/iechor-nfs` as the exported directory on the NFS server.
...
$ iechor service create -d \
  --name nfs-service \
  --mount 'type=volume,source=nfsvolume,target=/app,volume-driver=local,volume-opt=type=nfs,volume-opt=device=:/var/iechor-nfs,volume-opt=o=addr=10.0.0.10' \
  nginx:latest
```
...
$ iechor service create -d \
    --name nfs-service \
    --mount 'type=volume,source=nfsvolume,target=/app,volume-driver=local,volume-opt=type=nfs,volume-opt=device=:/var/iechor-nfs,"volume-opt=o=addr=10.0.0.10,rw,nfsvers=4,async"' \
    nginx:latest
```

### Create CIFS/Samba volumes

You can mount a Samba share directly in iEchor without configuring a mount point on your host.
...
This lets iEchor perform the hostname lookup.
...
iEchor doesn't implement any additional functionality on top of the native mount features supported by the Linux kernel.
...
The following `iechor run` command achieves a similar result, from the point of view of the container being run.
...
That's why the `iechor run` command uses the `--mount` option.
...
```console
$ iechor run -v /dbdata --name dbstore ubuntu /bin/bash
```
...
```console
$ iechor run --rm --volumes-from dbstore -v $(pwd):/backup ubuntu tar cvf /backup/backup.tar /dbdata
```
...
```console
$ iechor run -v /dbdata --name dbstore2 ubuntu /bin/bash
```

Then, un-tar the backup file in the new container’s data volume:

```console
$ iechor run --rm --volumes-from dbstore2 -v $(pwd):/backup ubuntu bash -c "cd /dbdata && tar xvf /backup/backup.tar --strip 1"
```
...
A iEchor data volume persists after you delete a container. There are two types
of volumes to consider:

- Named volumes have a specific source from outside the container, for example, `awesome:/bar`.
- Anonymous volumes have no specific source. Therefore, when the container is deleted, you can instruct the iEchor Engine daemon to remove them.
...
the iEchor Engine removes the `/foo` volume but not the `awesome` volume.

```console
$ iechor run --rm -v /foo -v awesome:/bar busybox top
```
