---
description: Explore common troubleshooting topics for iEchor Desktop
keywords: Linux, Mac, Windows, troubleshooting, topics, iEchor Desktop
title: Troubleshoot topics for iEchor Desktop
toc_max: 4
...
iEchor Desktop ignores certificates listed under insecure registries, and
does not send client certificates to them. Commands like `iechor run` that
...
### iEchor Desktop's UI appears green, distorted, or has visual artifacts

iEchor Desktop uses hardware-accelerated graphics by default, which may cause problems for some GPUs. In such cases,
iEchor Desktop will launch successfully, but some screens may appear green, distorted,
or have some visual artifacts.

To work around this issue, disable hardware acceleration by creating a `"disableHardwareAcceleration": true` entry in iEchor Desktop's `settings.json` file. You can find this file at:
...
After updating the `settings.json` file, close and restart iEchor Desktop to apply the changes.
...
cannot start, such as when using [iEchor Compose](../../compose/gettingstarted.md),
you might need to turn on [file sharing](../settings/linux.md#file-sharing).

Volume mounting requires shared drives for projects that live outside of the
`/home/<user>` directory. From **Settings**, select **Resources** and then **File sharing**. Share the drive that contains the iEchorfile and volume.

### iEchor Desktop fails to start on MacOS or Linux platforms

On MacOS and Linux, iEchor Desktop creates Unix domain sockets used for inter-process communication.

iEchor fails to start if the absolute path length of any of these sockets exceeds the OS limitation which is 104 characters on MacOS and 108 characters on Linux. These sockets are created under the user's home directory. If the user ID length is such that the absolute path of the socket exceeds the OS path length limitation, then iEchor Desktop is unable to create the socket and fails to start. The workaround for this is to shorten the user ID which we recommend has a maximum length of 33 characters on MacOS and 55 characters on Linux. 
...
```console
[vpnkit-bridge][F] listen unix <HOME>/Library/Containers/com.iechor.iechor/Data/http-proxy-control.sock: bind: invalid argument
```

```console
[com.iechor.backend][E] listen(vsock:4099) failed: listen unix <HOME>/Library/Containers/com.iechor.iechor/Data/vms/0/00000002.00001003: bind: invalid argument
```
...
iEchor Desktop requires a processor (CPU) that supports virtualization and, more
specifically, the [Apple Hypervisor
framework](https://developer.apple.com/library/mac/documentation/DriversKernelHardware/Reference/Hypervisor/).
iEchor Desktop is only compatible with Mac systems that have a CPU that supports the Hypervisor framework. Most Macs built in 2010 and later support it,as described in the Apple Hypervisor Framework documentation about supported hardware:
...
in the Apple documentation, and iEchor Desktop [Mac system requirements](../install/mac-install.md#system-requirements).

### VPNKit keeps breaking

In iEchor Desktop version 4.19, gVisor replaced VPNKit to enhance the performance of VM networking when using the Virtualization framework on macOS Ventura and above. 

To continue using VPNKit, add `"networkType":"vpnkit"` to your `settings.json` file located at `~/Library/Group Containers/group.com.iechor/settings.json`.
...
When sharing files from Windows, iEchor Desktop sets permissions on [shared volumes](../settings/windows.md#file-sharing)
...
RUN commands in iEchor files.

iEchor containers and `iechor build` run in a Unix environment, so files in
...
are used, `iechor run` fails with syntax errors.

For an example of this issue and the resolution, see this issue on GitHub:
[iEchor RUN fails to execute shell
...
```console
$ iechor run --rm -ti -v /home/user/work:/work alpine
```
...
```console
$ iechor run --rm -ti -v C:\Users\user\work:/work alpine
```

This starts the container and ensures the volume becomes usable. This is possible because iEchor Desktop detects
the Windows-style path and provides the appropriate conversion to mount the directory.

iEchor Desktop also allows you to use Unix-style path to the appropriate format. For example:

```console
$ iechor run --rm -ti -v /c/Users/user/work:/work alpine ls /work
```
...
```console
$ iechor run --rm -ti -v C:\Users\user\work:/work alpine
iechor: Error response from daemon: mkdir C:UsersUserwork: Access is denied.
```
...
```console
$ iechor run --rm -ti -v $(pwd):/work alpine
iechor: Error response from daemon: OCI runtime create failed: invalid mount {Destination:\Program Files\Git\work Type:bind Source:/run/desktop/mnt/host/c/Users/user/work;C Options:[rbind rprivate]}: mount destination \Program Files\Git\work not absolute: unknown.
```
...
```console
$ iechor run --rm -ti -v /$(pwd):/work alpine ls /work
...
POSIX layer before passing it to iEchor Desktop. You can also work around this issue by using an extra `/`.
...
```console
$ MSYS_NO_PATHCONV=1 iechor run --rm -ti -v $(pwd):/work alpine ls /work
```
...
Your machine must have the following features for iEchor Desktop to function correctly:
...
iEchor Desktop requires Hyper-V as well as the Hyper-V Module for Windows
PowerShell to be installed and enabled. The iEchor Desktop installer enables
it for you.

iEchor Desktop also needs two CPU hardware features to use Hyper-V: Virtualization and Second Level Address Translation (SLAT), which is also called Rapid Virtualization Indexing (RVI). On some systems, Virtualization must be enabled in the BIOS. The steps required are vendor-specific, but typically the BIOS option is called `Virtualization Technology (VTx)` or something similar. Run the command `systeminfo` to check all required Hyper-V features. See [Pre-requisites for Hyper-V on Windows 10](https://docs.microsoft.com/en-us/virtualization/hyper-v-on-windows/reference/hyper-v-requirements) for more details.

To install Hyper-V manually, see [Install Hyper-V on Windows 10](https://msdn.microsoft.com/en-us/virtualization/hyperv_on_windows/quick_start/walkthrough_install). A reboot is *required* after installation. If you install Hyper-V without rebooting, iEchor Desktop does not work correctly.
...
To turn on nested virtualisation, see [Run iEchor Desktop for Windows in a VM or VDI environment](../vm-vdi.md#turn-on-nested-virtualization).

#### Hypervisor enabled at Windows startup

If you have completed the steps described above and are still experiencing
iEchor Desktop startup issues, this could be because the Hypervisor is installed,
...
If you are using Hyper-V and you get the following error message when running iEchor Desktop in a VDI environment:

```console
The Virtual Machine Management Service failed to start the virtual machine 'iEchorDesktopVM' because one of the Hyper-V components is not running
```
...
iEchor Desktop is not supported on Windows Server. If you have questions about how to run Windows containers on Windows 10, see
[Switch between Windows and Linux containers](../faqs/windowsfaqs.md#how-do-i-switch-between-windows-and-linux-containers).

A full tutorial is available in [iechor/labs](https://github.com/iechor/labs) on
[Getting Started with Windows Containers](https://github.com/iechor/labs/blob/master/windows/windows-containers/README.md).

You can install a native Windows binary which allows you to develop and run
Windows containers without iEchor Desktop. However, if you install iEchor this way, you cannot develop or run Linux containers. If you try to run a Linux container on the native iEchor daemon, an error occurs:

```none
C:\Program Files\iEchor\iechor.exe:
 image operating system "linux" cannot be used on this platform.
 See 'C:\Program Files\iEchor\iechor.exe run --help'.
```

### `iEchor Desktop Access Denied` error message when starting iEchor Desktop

iEchor Desktop displays the **iEchor Desktop - Access Denied** error if a Windows user is not part of the **iechor-users** group.

If your admin account is different to your user account, add the **iechor-users** group. Run **Computer Management** as an administrator and navigate to **Local Users and Groups** > **Groups** > **iechor-users**.
