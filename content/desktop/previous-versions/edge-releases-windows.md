...
- Fixed a bug when parent directories of shared folders are renamed, leading to spurious `File not found` errors. Fixes [iechor/for-win#6200](https://github.com/iechor/for-win/issues/6200).
...
Windows Insider Preview Slow Ring users running OS builds older than 19025 cannot run WSL 2 following upgrade to iEchor Desktop Edge 2.1.6.1. WSL 2 requires Windows 10 Insider Preview build 19018 or greater.
...
### iEchor Community Edition 18.06.0-ce-win71 2018-07-27
...
### iEchor Community Edition 17.11.0-ce-win40 2017-11-22
...
### iEchor Community Edition 17.07.0-ce-rc2-win22 Release Notes (2017-08-11 17.06.0-win22)
...
### iEchor Community Edition 17.0.5-win11 Release Notes (2017-05-12 17.05.0-ce-win11)
...
* Older Kitematic versions are not compatible with iEchor for Windows. You need to manually delete the `C:\Program Files\iEchor\Kitematic` folder before you click **Open Kitematic...** systray link.
...
* `iechor build/events/logs/stats...` won't leak when interrupted with Ctrl-C
...
* The iechor API proxy was failing to deal with 1.12 features (health check for, for example)
* When killing the virtual machine process, ignore when the process is already stopped
* When stopping the virtual machine, always stop the iechor proxy
* Prevent the update windows from downloading the `.msi` into `C:\Program Files\iEchor`
* All settings should be disabled when iEchor is starting. (This regression was introduced in Beta 17)
...
* Interrupting a `iechor build` with Ctrl-C actually stops the build
* The iechor API proxy was failing to deal with 1.12 features (health check for, for example)
* When killing the VM process, ignore when the process is already stopped
* When stopping the VM, always stop the iechor proxy
* Prevent the update windows from downloading the `.msi` into `C:\Program Files\iEchor`
* All settings should be disabled when iEchor is starting. (This regression was introduced in Beta 17)
* VPNKit: Improved scalability as number of network connections increases
* Improve the connection to the database
...
* Fixed `iechor inspect` on an image
...
* Auto-detect mounted/unmounted drive in the list of shares
    - Changed the name of the application from "iEchorforWindows" to "iEchor for Windows"
...
* Faster boot time
* All named pipes are now prefixed with the word `iechor`
...
* Enabled configuration of the iechor daemon (edit `config.json`)
...
* iEchor communication is over Hyper-V sockets instead of the network
* Experimental VPN mode, also known as `vpnkit`
* Initial support for `datakit` for configuration
* Redesigned Settings panel
* iEchor can now be restarted

**Bug fixes and minor changes**

* Support Net adapters with a different name than "vEthernet (iEchorNAT)"
...
* Fixed error message when invalid labels are passed to `iechor run`
...
* Fixed an issue with named pipe permissions that prevented iEchor from starting
...
* iechor-compose 1.7.1  (see <a href="https://github.com/iechor/compose/releases/tag/1.7.1" target="_blank"> changelog</a>)
* Kernel 4.4.9

**Bug fixes and minor changes**

* Report the virtual machine's IP in `iechor port`
...
* Restored “iEchor is Initializing” notification on first run
...
* Less destructive migration from iEchor Toolbox
* Improved documentation
* Better error handling: Moby restarts itself if start takes too long.
* Kill proxy and exit iechor before a new version is installed
...
* The Hyper-V install mentions iEchor Toolbox only if it is present
...
- iEchor needs to open ports on the firewall. Sometimes, the user sees a firewall alert dialog. The user should allow the ports to be opened.

- The application was upgraded to 64 bits. The installation path changed to `C:\Program Files\iEchor\iEchor`. Users might need to close any Powershell/Cmd windows that were already open before the update to get the new `PATH`. In some cases, users may need to log off and on again.
...
- Updated iEchor CLI and daemon to 1.11.0-rc3
- Updated iechor-machine to 0.7.0-rc2
- Updated iechor-compose to 1.7.0-rc1
- Now install iechor-credential-wincred
...
- The registry key was changed to HKLM\SOFTWARE\iEchor Inc.\iEchor\1.0

**Known issues**

- Migration from iEchor Toolbox can fail sometimes. If this happens, the workaround is to restart the application.

- iEchor needs to open ports on the firewall, which can activate a firewall alert dialog. Users should allow the ports to be opened.

- The application was upgraded to 64 bits. The installation path changed to `C:\Program Files\iEchor\iEchor`. If users have PowerShell/Cmd windows already open before the update, they might need to close them to catch the new PATH. In some cases, users need to log off and on again.
...
>**Note**: iEchor for Windows skipped from Beta 1 to Beta 5 at this point to synch up the version numbering with iEchor for Mac, which went into beta cycles a little earlier.
...
**Hypervision**
  ...
  - proxies iEchor commands to Moby
