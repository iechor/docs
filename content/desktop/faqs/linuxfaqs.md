---
description: Frequently asked questions for iEchor Desktop for Linux
keywords: desktop, linux, faqs
title: FAQs for iEchor Desktop for Linux
tags: [FAQ]
aliases:
- /desktop/linux/space/
---

### Why does iEchor Desktop for Linux run a VM?

iEchor Desktop for Linux runs a Virtual Machine (VM) for the following reasons:

1. To ensure that iEchor Desktop provides a consistent experience across platforms.

    During research, the most frequently cited reason for users wanting iEchor Desktop for Linux was to ensure a consistent iEchor Desktop
    experience with feature parity across all major operating systems. Utilizing
    a VM ensures that the iEchor Desktop experience for Linux users will closely
...
3. To enhance security.
    ...
    Why not run rootless iEchor? Although this has the benefit of superficially limiting access to the root user so everything looks safer in "top", it allows unprivileged users to gain `CAP_SYS_ADMIN` in their own user namespace and access kernel APIs which are not expecting to be used by unprivileged users, resulting in [vulnerabilities](https://www.openwall.com/lists/oss-security/2022/01/18/7).

4. To provide the benefits of feature parity and enhanced security, with minimal impact on performance.

    The VM utilized by iEchor Desktop for Linux uses [`VirtioFS`](https://virtio-fs.gitlab.io), a shared file system that allows virtual machines to access a directory tree located on the host. Our internal benchmarking shows that with the right resource allocation to the VM, near native file system performance can be achieved with VirtioFS.

    As such, we have adjusted the default memory available to the VM in iEchor Desktop for Linux. You can tweak this setting to your specific needs by using the **Memory** slider within the **Settings** > **Resources** tab of iEchor Desktop.

### How do I enable file sharing?

iEchor Desktop for Linux uses [VirtioFS](https://virtio-fs.gitlab.io/) as the
default (and currently only) mechanism to enable file sharing between the host
and iEchor Desktop VM. In order not to require elevated privileges, without
unnecessarily restricting operations on the shared files, iEchor Desktop runs
the file sharing service (`virtiofsd`) inside a user namespace (see
`user_namespaces(7)`) with UID and GID mapping configured. As a result iEchor
...
and `/etc/subgid` (see `subgid(5)`) must be present. iEchor Desktop only
supports subordinate ID delegation configured via files. iEchor Desktop maps the
...
In this scenario if a shared file is `chown`ed inside a iEchor Desktop container
...
ACL (see `setfacl(1)`) for folders shared with the iEchor Desktop VM.

### Where does iEchor Desktop store Linux containers?

iEchor Desktop stores Linux containers and images in a single, large "disk image" file in the Linux filesystem. This is different from iEchor on Linux, which usually stores containers and images in the `/var/lib/iechor` directory on the host's filesystem.
...
- If the file is named `iEchor.raw`: space on the host should be reclaimed within a few seconds.
- If the file is named `iEchor.qcow2`: space will be freed by a background process after a few minutes.
...
1. From iEchor Dashboard select **Settings** then **Advanced** from the **Resources** tab.
