...
In your `iechor-php-sample` directory, create a file named
`iechor-php-kubernetes.yaml`. Open the file in an IDE or text editor and add
...
1. In a terminal, navigate to the `iechor-php-sample` directory
   and deploy your application to Kubernetes.

   ```console
   $ kubectl apply -f iechor-php-kubernetes.yaml
   ```

   You should see output that looks like the following, indicating your Kubernetes objects were created successfully.

   ```text
   deployment.apps/iechor-php-demo created
...
4. Run the following command to tear down your application.
   ...
   ```console
   $ kubectl delete -f iechor-php-kubernetes.yaml
   ```
