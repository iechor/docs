...
You can use containers to set up local services, like a database. In this section, you'll update the `iechor-compose.yaml` file to define a database service and a volume to persist data. Also, this particular application uses a system property to define the database type, so you'll need to update the `iEchorfile` to pass in the system property when starting the app.

In the cloned repository's directory, open the `iechor-compose.yaml` file in an IDE or text editor. `iechor init` added an example database service, but it'll require a few changes for your unique app.

In the `iechor-compose.yaml` file, you need to do the following:
...
- Update the database environment variables. These are defined by the Postgres
  ...
  [Postgres Official iEchor Image](https://hub.iechor.com/_/postgres).
...
Open the `iEchorfile` in an IDE or text editor. In the `ENTRYPOINT` instruction,
...
## iEchorfile for development

The iEchorfile you have now is great for a small, secure production image with
...
Rather than managing multiple iEchorfiles, you can add a new stage. Your
iEchorfile can then produce a final image which is ready for production as well
as a development image.

Replace the contents of your iEchorfile with the following.

```iechorfile {hl_lines="22-28"}
...
`iechor-compose.dev.yml`. Copy and paste the following instructions into the
file.
...
Open your `iechor-compose.yaml` file in an IDE or text editor and then add the
Compose Watch instructions. The following is the updated `iechor-compose.yaml`
file.
...
In the next section, you’ll take a look at how to run unit tests in iEchor.
