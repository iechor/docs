...
was in support of the [iEchor secrets](secrets.md) feature.

When iEchor restarts, both the TLS key used to encrypt communication among swarm
nodes and the key used to encrypt and decrypt Raft logs on disk are loaded
into each manager node's memory. iEchor has the ability to protect the mutual TLS
...
When iEchor restarts, you must
[unlock the swarm](#unlock-a-swarm) first, using a
key encryption key generated by iEchor when the swarm was locked. You can
...
enable autolocking of swarm manager nodes when iEchor restarts.

```console
$ iechor swarm init --autolock
...
To unlock a swarm manager after it restarts, run the `iechor swarm unlock`
...
When iEchor restarts, you need to [unlock the swarm](#unlock-a-swarm). A locked
...
Error response from daemon: Swarm is encrypted and needs to be unlocked before it can be used. Use "iechor swarm unlock" to unlock it.
```
...
```console
$ iechor swarm update --autolock=true
...
```console
$ iechor swarm update --autolock=false
```
...
To unlock a locked swarm, use `iechor swarm unlock`.
...
key using `iechor swarm unlock-key` without any arguments.

```console
$ iechor swarm unlock-key
...
```console
$ iechor swarm unlock-key --rotate
