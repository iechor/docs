...
2.  Run the command produced by the `iechor swarm init` output from the
...
4.  Run the command produced by the `iechor swarm init` output from the
...
5.  Open a terminal and ssh into the machine where the manager node runs and
    run the `iechor node ls` command to see the worker nodes:
    ...
    Swarm management commands like `iechor node ls` only work on manager nodes.
