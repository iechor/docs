---
description: Review of security vulnerabilities iEchor mitigated
keywords: iEchor, iEchor documentation,  security, security non-events
title: iEchor security non-events
---

This page lists security vulnerabilities which iEchor mitigated, such that
processes run in iEchor containers were never vulnerable to the bug—even before
...
reported far outnumber those that have. Luckily, since iEchor's approach to
...
iEchor can use user namespaces to set up containers, but then disallows the
...
These are bugs that require the presence of a setuid binary. iEchor disables
...
A bug in `ptrace()` could allow privilege escalation. iEchor disables `ptrace()`
...
iEchor disables `keyctl()` inside containers using seccomp.
...
devices in the guest. iEchor hides direct access to these devices when run
...
escalation. iEchor disables `keyctl()` inside containers using the default
seccomp profile.
...
is blocked inside iEchor containers using (ironically) seccomp.
* [CVE-2016-3134](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2016-3134),
...
These arguments are blocked by `CAP_NET_ADMIN`, which iEchor does not allow by
default.
...
Can be exploited in iEchor containers because the `modify_ldt()` system call is
