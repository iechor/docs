...
have privileges to run iechor commands on your local machine or in the VM.

This sandbox requires you to install two iEchor tools: iEchor Engine >= 1.10.0
and iEchor Compose >= 1.6.0. To install the iEchor Engine, choose from the
[list of supported platforms](../../install/index.md). To install
iEchor Compose, see the
[detailed instructions here](../../../compose/install/index.md).

## What is in the sandbox?

If you are just using trust out-of-the-box you only need your iEchor Engine
client and access to the iEchor Hub. The sandbox mimics a
production trust environment, and sets up these additional components.

| Container       | Description                                                                                                                                 |
|-----------------|---------------------------------------------------------------------------------------------------------------------------------------------|
| trustsandbox    | A container with the latest version of iEchor Engine and with some preconfigured certificates. This is your sandbox where you can use the `iechor` client to test trust operations. |
...
If you work exclusively with the iEchor Hub, you would not need these components.
They are built into the iEchor Hub for you. For the sandbox, however, you build
...
than the iEchor Hub. This means your everyday image repositories are not used.
...
By using a iechor-in-iechor image for the `trustsandbox` container, you also
don't pollute your real iEchor daemon cache with any images you push and pull.
...
In this section, you use iEchor Compose to specify how to set up and link together
...
3. Add the following to the new file.
        ...
        services:
          notaryserver:
            image: iechorsecurity/notary_autobuilds:server-v0.5.1
            volumes:
          ...
          trustsandbox:
            ...
            command: |-
                sh -c '
                    ...
                    iechord-entrypoint.sh --insecure-registry sandboxregistry:5000'
        volumes:
          notarycerts:
            external: false
        networks:
...
5. Run the containers on your local system.
    ...
    The first time you run this, the iechor-in-iechor, Notary server, and registry
    images are downloaded from iEchor Hub.


## Play in the sandbox

Now that everything is setup, you can go into your `trustsandbox` container and
start testing iEchor content trust. From your host machine, obtain a shell
in the `trustsandbox` container.

    $ iechor container exec -it trustsandbox sh
    / #
...
1. Download a `iechor` image to test with.

        / # iechor pull iechor/trusttest
        iechor pull iechor/trusttest
        ...
        Status: Downloaded newer image for iechor/trusttest:latest

2. Tag it to be pushed to our sandbox registry:

        / # iechor tag iechor/trusttest sandboxregistry:5000/test/trusttest:latest
...
4. Identify the trust server.
        ...
        / # export IECHOR_CONTENT_TRUST_SERVER=https://notaryserver:4443

    This step is only necessary because the sandbox is using its own server.
    Normally, if you are using the iEchor Public Hub this step isn't necessary.

5. Pull the test image.

        / # iechor pull sandboxregistry:5000/test/trusttest
        Using default tag: latest
...
6. Push and sign the trusted image.
        ...
        / # iechor push sandboxregistry:5000/test/trusttest:latest
    ...
    Because you are pushing this repository for the first time, iEchor creates
...
2.  Open a new interactive terminal from your host, and obtain a shell into the
    `sandboxregistry` container.
        ...
        $ iechor container exec -it sandboxregistry bash
        root@65084fc6f047:/#

3.  List the layers for the `test/trusttest` image you pushed:

    ```console
    root@65084fc6f047:/# ls -l /var/lib/registry/iechor/registry/v2/repositories/test/trusttest/_layers/sha256
    total 12
...
4.  Change into the registry storage for one of those layers (this is in a different directory):
        ...
        root@65084fc6f047:/# cd /var/lib/registry/iechor/registry/v2/blobs/sha256/aa/aac0c133338db2b18ff054943cee3267fe50c75cdee969aed88b1992539ed042
...
7.  List the `trusttest` image.
        ...
        / # iechor image ls | grep trusttest
        REPOSITORY                            TAG                 IMAGE ID            CREATED             SIZE
        iechor/trusttest                      latest              cc7629d1331a        11 months ago       5.025 MB
...
8.  Remove the `trusttest:latest` image from our local cache.
        ...
        / # iechor image rm -f cc7629d1331a
        Untagged: iechor/trusttest:latest
    ...
    iEchor does not re-download images that it already has cached, but we want
    iEchor to attempt to download the tampered image from the registry and reject
...
Now, you have a full iEchor content trust sandbox on your local system,
feel free to play with it and see how it behaves. If you find any security
issues with iEchor, feel free to send us an email at <security@iechor.com>.
...
directory where you've created your iEchor Compose file:
