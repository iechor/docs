...
keywords: automated, build, images, iEchor Hub
title: Set up Automated Builds
aliases:
- /iechor-hub/builds/automated-build/
- /iechor-cloud/feature-reference/automated-build/
- /iechor-cloud/builds/automated-build/
- /iechor-cloud/builds/
- /iechor-hub/builds/classic/
---
...
You can configure repositories in iEchor Hub so that they automatically
...
4. Select the **source repository** to build the iEchor images from.
...
6. Review the default **Build Rules**.
    ...
    Build rules control what iEchor Hub builds into images from the contents
    of the source code repository, and how the resulting images are tagged
    within the iEchor repository.
    ...
    `master` or `main`, and creates a iEchor image tagged with `latest`. For more information, see [set up build rules](#set-up-build-rules)
...
8. For each branch or tag, enable or disable the **Autobuild** toggle.
    ...
    level), but the built iEchor image isn't pushed to the repository.

9. For each branch or tag, enable or disable the **Build Caching** toggle.

    [Build caching](../../develop/develop-images/iechorfile_best-practices.md#leverage-build-cache)
...
10. Select **Save** to save the settings, or select **Save and build** to save and
   run an initial test.
    ...
    > iEchor Hub on every push. Only pushes to branches that are listed as the
...
repository, and builds the `master` or `main` branch into a iEchor image tagged with
`latest`.
...
* Enter the name of the **Source** branch or tag you want to build.
  ...
  `master`, and creates a iEchor image tagged with `latest`.

  You can also use a regex to select which source branches or tags to build.
  To learn more, see
  [regexes](#regexes-and-automated-builds).

* Enter the tag to apply to iEchor images built from this source.
...
* Specify the **iEchorfile location** as a path relative to the root of the source code repository. If the iEchorfile is at the repository root, leave this path set to `/`.

> **Note**
>
> When iEchor Hub pulls a branch from a source code repository, it performs a
...
When you set variable values from the iEchor Hub UI, you can use them by the
commands you set in `hooks` files. However, they're stored so that only users who have `admin` access to the iEchor Hub repository can see their values. This
...
destination iEchor tag to set up an automated build. You can also:

- Change where the build looks for the iEchorfile
...
4. Enter the tag to apply to iEchor images built from this source.
...
### Set the build context and iEchorfile location
...
> it blank, the build system uses the path to the iEchorfile as the build
> context. However, to avoid confusion it's recommended that you specify the
> complete path.

You can specify the **iEchorfile location** as a path relative to the build
context. If the iEchorfile is at the root of the build context path, leave the
iEchorfile path set to `/`. If the build context field is blank, set the path
to the iEchorfile from the root of the source repository.

### Regexes and Automated builds

You can specify a regular expression (regex) so that only matching branches or
tags are built. You can also use the results of the regex to create the iEchor
...
these in the **iEchor Tag** field using `{\1}` through `{\9}`.
...
iEchor build system, add the [environment variable](index.md#environment-variables-for-builds)
`IECHOR_BUILDKIT=0`. Refer to the [BuildKit](../../build/buildkit/index.md)
...
These same actions are also available for team repositories from iEchor Hub if
...
When you set up Automated builds for teams, you grant iEchor Hub access to
your source code repositories using OAuth tied to a specific user account. This
means that iEchor Hub has access to everything that the linked source provider
account can access.
...
5. Sign in to iEchor Hub as an owner, switch to the organization, and follow the instructions to [link to source code repository](link-source.md) using the service account.
