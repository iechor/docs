...
description: "API Documentation for iEchor"
keywords: "API, iEchor, rcli, REST, documentation"
aliases:
- /engine/reference/api/iechor_remote_api_v1.24/
- /reference/api/iechor_remote_api_v1.24/
---
...
## 1. Brief introduction
 ...
 - The daemon listens on `unix:///var/run/iechor.sock` but you can
   [Bind iEchor to another host/port or a Unix socket](https://docs.iechor.com/engine/reference/commandline/iechord/#bind-iechor-to-another-host-port-or-a-unix-socket).
...
**Example response**:
...

	{
		...
		"HostnamePath": "/var/lib/iechor/containers/ba033ac4401106a3b513bc9d639eee123ad78ca3616b921167cd74b20e25ed39/hostname",
		"HostsPath": "/var/lib/iechor/containers/ba033ac4401106a3b513bc9d639eee123ad78ca3616b921167cd74b20e25ed39/hosts",
		"LogPath": "/var/lib/iechor/containers/1eb5fabf5a03807136561b3c00adcd2992b535d624d5e18b6cdc6a6844d9767b/1eb5fabf5a03807136561b3c00adcd2992b535d624d5e18b6cdc6a6844d9767b-json.log",
		...
		"Path": "/bin/sh",
		"ProcessLabel": "",
		"ResolvConfPath": "/var/lib/iechor/containers/ba033ac4401106a3b513bc9d639eee123ad78ca3616b921167cd74b20e25ed39/resolv.conf",
		"RestartCount": 1,
		"State": {
			"Error": "",
			"ExitCode": 9,
...
**Example response**:
     ...
     Content-Type: application/vnd.iechor.raw-stream
     Connection: Upgrade
     Upgrade: tcp
...
See the description of the `X-iEchor-Container-Path-Stat` header in the
following section.
...
**Example response**:
    ...
    X-iEchor-Container-Path-Stat: eyJuYW1lIjoicm9vdCIsInNpemUiOjQwOTYsIm1vZGUiOjIxNDc0ODQwOTYsIm10aW1lIjoiMjAxNC0wMi0yN1QyMDo1MToyM1oiLCJsaW5rVGFyZ2V0IjoiIn0=

    {% raw %}
    {{ TAR STREAM }}
    {% endraw %}

On success, a response header `X-iEchor-Container-Path-Stat` will be set to a
...
See the `iechor run` and `iechor build` commands for examples of digest and tag
...
#### Build image from a iEchorfile

`POST /build`

Build an image from a iEchorfile
...
`iEchorfile` at the archive's root. The `iechorfile` parameter may be
used to specify a different build instructions file. To do this, its value must be
the path to the alternate build instructions file to use.

The archive may include any number of other files,
which are accessible in the build context (See the [*ADD build
command*](https://docs.iechor.com/engine/reference/builder/#add)).

The iEchor daemon performs a preliminary validation of the `iEchorfile` before
...
-   **iechorfile** - Path within the build context to the `iEchorfile`. This is
        ignored if `remote` is specified and points to an external `iEchorfile`.
...
-   **remote** – A Git repository URI or HTTP/HTTPS context URI. If the
        ...
        a file called `iEchorfile` and the image is built from that file. If
        ...
        points to a tarball and the `iechorfile` parameter is also specified,
...
-   **buildargs** – JSON map of string pairs for build-time variables. Users pass
        these values at build-time. iEchor uses the `buildargs` as the environment
        context for command(s) run via the iEchorfile's `RUN` instruction or for
        variable expansion in other iEchorfile instructions. This is not meant for
        passing secret values. [Read more about the buildargs instruction](https://docs.iechor.com/engine/reference/builder/#arg)
-   **shmsize** - Size of `/dev/shm` in bytes. The size must be greater than 0.  If omitted the system uses 64MB.
...
-   **X-Registry-Config** – A base64-url-safe-encoded Registry Auth Config JSON
    ...
    (for legacy reasons) the "official" iEchor, Inc. hosted registry must
    be specified with both a "https://" prefix and a "/v1/" suffix even
    though iEchor will prefer to use the v2 registry API.
...
Search for an image on [iEchor Hub](https://hub.iechor.com).

> **Note**:
> The response keys have changed from API v1.6 to reflect the JSON
> sent by the registry server to the iechor daemon's request.
...
**Example request**:
...

    {
         "username": "hannibal",
         "password": "xxxx",
         "serveraddress": "https://index.iechor.io/v1/"
    }
...
**Example response**:
...

    {
        ...
        "IPv4Forwarding": true,
        "Images": 16,
        "IndexServerAddress": "https://index.iechor.io/v1/",
        "InitPath": "/usr/bin/iechor",
        "InitSha1": "",
        "KernelMemory": true,
...
#### Show the iechor version information
...
-   **changes** – iEchorfile instructions to apply while committing
...
#### Monitor iEchor's events

`GET /events`

Get container events from iechor, in real time via streaming.

iEchor containers report the following events:

    attach, commit, copy, create, destroy, detach, die, exec_create, exec_detach, exec_start, export, health_status, kill, oom, pause, rename, resize, restart, start, stop, top, unpause, update

iEchor images report the following events:

    delete, import, load, pull, push, save, tag, untag

iEchor volumes report the following events:

    create, mount, unmount, destroy

iEchor networks report the following events:

    create, connect, disconnect, destroy

iEchor daemon report the following event:
...
#### Load a tarball with a set of images and tags into iechor

`POST /images/load`

Load a set of images and tags into a iEchor repository.
See the [image tarball format](#image-tarball-format) for more details.
...
- `json`: detailed layer information, similar to `iechor inspect layer_id`
      ...
      "Volumes": [
        {
          "Name": "tardis",
          "Driver": "local",
          "Mountpoint": "/var/lib/iechor/volumes/tardis",
          "Labels": null,
          "Scope": "local"
        }
      ],
...
- **Name** - The new volume's name. If not specified, iEchor generates a name.
...
**Example response**:
...

    {
      "Name": "tardis",
      "Driver": "custom",
      "Mountpoint": "/var/lib/iechor/volumes/tardis/_data",
      "Status": {
        "hello": "world"
      },
      "Labels": {
...
<!-- TODO Document "iechor plugin push" endpoint once we have "plugin build"
        ...
        "NetworksAttachments": [
          {
            "Network": {
              ...
              "Spec": {
                "Name": "ingress",
                "Labels": {
                  "com.iechor.swarm.internal": "true"
                },
                "DriverConfiguration": {},
                "IPAMOptions": {
                  "Driver": {},
              ...
              "DriverState": {
                "Name": "overlay",
                "Options": {
                  "com.iechor.network.driver.overlay.vxlanid_list": "256"
                }
              },
              "IPAMOptions": {
                "Driver": {
...
### 4.1 Inside `iechor run`

As an example, the `iechor run` command line makes the following API calls:
...
To hint potential proxies about connection hijacking, iEchor client sends
connection upgrade headers similarly to websocket.

    Upgrade: tcp
    Connection: Upgrade

When iEchor daemon detects the `Upgrade` header, it switches its status code
...
`--api-cors-header` when running iEchor in daemon mode. Set * (asterisk) allows all,
default or blank means CORS disabled

    $ iechord -H="192.168.1.9:2375" --api-cors-header="http://foo.bar"
