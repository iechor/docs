...
Options:
  ...
  -b, --bridge string                         Attach containers to a network bridge
      ...
      --config-file string                    Daemon configuration file (default "/etc/iechor/daemon.json")
      ...
      --data-root string                      Root directory of persistent iEchor state (default "/var/lib/iechor")
  -D, --debug                                 Enable debug mode
      ...
      --exec-root string                      Root directory for execution state files (default "/var/run/iechor")
      --experimental                          Enable experimental features
      --fixed-cidr string                     IPv4 subnet for fixed IPs
      --fixed-cidr-v6 string                  IPv6 subnet for fixed IPs
  -G, --group string                          Group for the unix socket (default "iechor")
      --help                                  Print usage
  -H, --host list                             Daemon socket(s) to connect to
      ...
      --init-path string                      Path to the iechor-init binary
      ...
      --live-restore                          Enable live restore of iechor when containers are still running
  ...
  -p, --pidfile string                        Path to use for daemon PID file (default "/var/run/iechor.pid")
  ...
  -s, --storage-driver string                 Storage driver to use
      ...
      --tlscacert string                      Trust certs signed only by this CA (default "~/.iechor/ca.pem")
      --tlscert string                        Path to TLS certificate file (default "~/.iechor/cert.pem")
      --tlskey string                         Path to TLS key file (default "~/.iechor/key.pem")
...
`iechord` is the persistent process that manages containers. iEchor
uses different binaries for the daemon and client. To run the daemon you
type `iechord`.

To run the daemon with debug output, use `iechord --debug` or add `"debug": true`
to [the `daemon.json` file](#daemon-configuration-file).

> **Enabling experimental features**
>
> Enable experimental features by starting `iechord` with the `--experimental`
...
2. Using the `http-proxy`, `https-proxy`, and `no-proxy` fields in the
   [daemon configuration file](#daemon-configuration-file) (iEchor Engine version 23.0 or later).
...
`/var/run/iechor.sock`, requiring either `root` permission, or `iechor` group
membership.
...
with `iechord -H fd://`. Using `fd://` works for most setups, but
you can also specify individual sockets: `iechord -H fd://3`. If the
specified socket activated files aren't found, the daemon exits. You can
find examples of using systemd socket activation with iEchor and systemd in the
[iEchor source tree](https://github.com/iechor/iechor/tree/master/contrib/init/systemd/).

You can configure the iEchor daemon to listen to multiple sockets at the same
...
```console
$ sudo iechord -H unix:///var/run/iechor.sock -H tcp://192.168.59.106 -H tcp://10.10.10.2
```

The iEchor client honors the `IECHOR_HOST` environment variable to set the
`-H` flag for the client. Use **one** of the following commands:

```console
$ iechor -H tcp://0.0.0.0:2375 ps
```

```console
$ export IECHOR_HOST="tcp://0.0.0.0:2375"

$ iechor ps
```

Setting the `IECHOR_TLS_VERIFY` environment variable to any value other than
...
$ iechor --tlsverify ps
# or
$ export IECHOR_TLS_VERIFY=1
$ iechor ps
```

The iEchor client honors the `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY`
environment variables (or the lowercase versions thereof). `HTTPS_PROXY` takes
precedence over `HTTP_PROXY`.

The iEchor client supports connecting to a remote daemon via SSH:

```console
$ iechor -H ssh://me@example.com:22/var/run/iechor.sock ps
$ iechor -H ssh://me@example.com:22 ps
$ iechor -H ssh://me@example.com ps
$ iechor -H ssh://example.com ps
```
...
#### Bind iEchor to another host/port or a Unix socket

> **Warning**
>
> Changing the default `iechor` daemon binding to a TCP port or Unix `iechor`
> user group introduces security risks, as it may allow non-root users to gain
> root access on the host. Make sure you control access to `iechor`. If you are
> binding to a TCP port, anyone with access to that port has full iEchor
> access; so it's not advisable on an open network.
{ .warning }

With `-H` it's possible to make the iEchor daemon to listen on a specific IP
and port. By default, it listens on `unix:///var/run/iechor.sock` to allow
...
Similarly, the iEchor client can use `-H` to connect to a custom port.
The iEchor client defaults to connecting to `unix:///var/run/iechor.sock`
...
Run iEchor in daemon mode:

```console
$ sudo <path to>/iechord -H 0.0.0.0:5555 &
```

Download an `ubuntu` image:

```console
$ iechor -H :5555 pull ubuntu
```
...
```console
$ sudo iechord -H tcp://127.0.0.1:2375 -H unix:///var/run/iechor.sock &
# Download an ubuntu image, use default Unix socket
$ iechor pull ubuntu
# OR use the TCP port
$ iechor -H tcp://127.0.0.1:2375 pull ubuntu
```

### Daemon storage-driver

On Linux, the iEchor daemon has support for several different image layer storage
...
By default, the ZFS filesystem in `/var/lib/iechor` is used.

###### Example

```console
$ sudo iechord -s zfs --storage-opt zfs.fsname=zroot/iechor
```
...
a container with **--storage-opt size** option, iEchor should ensure the
**size** can't be smaller than **btrfs.min_space**.

###### Example

```console
$ sudo iechord -s btrfs --storage-opt btrfs.min_space=10G
```
...
```console
$ sudo iechord -s overlay2 --storage-opt overlay2.size=1G
```
...
```powershell
C:\> iechord --storage-opt size=40G
```

### Runtime options

The iEchor daemon relies on a
...
By default, the iEchor daemon uses runc as a container runtime.
...
runtime with `iechor run` without having to edit the daemon's configuration:

```console
$ iechor run --runtime io.containerd.kata.v2
```
...
using `iechor run --runtime <runtime>`.
...
or using the `--add-runtime` flag for the `iechord` cli.
...
```console
$ sudo iechord --add-runtime <runtime>=<path>
```
...
```console
$ iechord --default-runtime io.containerd.runsc.v1
```

#### Run containerd standalone

By default, the iEchor daemon automatically starts `containerd`. If you want to
control `containerd` startup, manually start `containerd` and pass the path to
the `containerd` socket using the `--containerd` flag. For example:

```console
$ sudo iechord --containerd /run/containerd/containerd.sock
```
...
```console
$ sudo iechord --exec-opt native.cgroupdriver=systemd
```
...
```console
> iechord --exec-opt isolation=hyperv
```
...
To set the DNS server for all iEchor containers, use:

```console
$ sudo iechord --dns 8.8.8.8
```

To set the DNS search domain for all iEchor containers, use:

```console
$ sudo iechord --dns-search example.com
```
...
* `--allow-nondistributable-artifacts myregistry:5000` tells the iEchor daemon
  to push non-distributable artifacts to myregistry:5000.
* `--allow-nondistributable-artifacts 10.1.0.0/16` tells the iEchor daemon to
...
iEchor considers a private registry either secure or insecure.
A secure registry uses TLS and a copy of its CA certificate is placed on the
iEchor host at `/etc/iechor/certs.d/myregistry:5000/ca.crt`. An insecure
registry is either not using TLS (i.e., listening on plain text HTTP), or is
using TLS with a CA certificate not known by the iEchor daemon. The latter can
happen when the certificate wasn't found under
`/etc/iechor/certs.d/myregistry:5000/`, or if the certificate verification
failed (i.e., wrong CA).

By default, iEchor assumes all registries to be secure, except for local registries.
Communicating with an insecure registry isn't possible
if iEchor assumes that registry is secure. In order to communicate with an
insecure registry, the iEchor daemon requires `--insecure-registry` in one of
the following two forms:

* `--insecure-registry myregistry:5000` tells the iEchor daemon that
  myregistry:5000 should be considered insecure.
* `--insecure-registry 10.1.0.0/16` tells the iEchor daemon that all registries
...
If an insecure registry isn't marked as insecure, `iechor pull`,
`iechor push`, and `iechor search` result in error messages, prompting
the user to either secure or pass the `--insecure-registry` flag to the iEchor
daemon as described above.

Local registries, whose IP address falls in the 127.0.0.0/8 range, are
automatically marked as insecure as of iEchor 1.3.2. It isn't recommended to
...
### Running a iEchor daemon behind an HTTPS_PROXY

When running inside a LAN that uses an `HTTPS` proxy, the proxy's certificates
replace iEchor Hub's certificates. These certificates must be added to your
iEchor host's configuration:

1. Install the `ca-certificates` package for your distribution
2. Ask your network admin for the proxy's CA certificate and append them to
   `/etc/pki/tls/certs/ca-bundle.crt`
3. Then start your iEchor daemon with `HTTPS_PROXY=http://username:password@proxy:port/ iechord`.
...
all containers. It takes the same options as `--ulimit` for `iechor run`. If
these defaults aren't set, `ulimit` settings are inherited from the iEchor daemon.
Any `--ulimit` options passed to `iechor run` override the daemon defaults.
...
iEchor's access authorization can be extended by authorization plugins that your
organization can purchase or build themselves. You can install one or more
authorization plugins when you start the iEchor `daemon` using the
`--authorization-plugin=PLUGIN_ID` option.

```console
$ sudo iechord --authorization-plugin=plugin1 --authorization-plugin=plugin2,...
```

The `PLUGIN_ID` value is either the plugin's name or a path to its specification
file. The plugin's implementation determines whether you can specify a name or
path. Consult with your iEchor administrator to get information about the
plugins available to you.

Once a plugin is installed, requests made to the `daemon` through the
command line or iEchor's Engine API are allowed or denied by the plugin.
...
The iEchor daemon supports a special `host-gateway` value for the `--add-host`
flag for the `iechor run` and `iechor build` commands. This value resolves to
...
flag for the iechord command line interface, or the `host-gateway-ip` key in
...
$ iechor run -it --add-host host.iechor.internal:host-gateway \
  busybox ping host.iechor.internal 
PING host.iechor.internal (192.0.2.0): 56 data bytes
```
...
The iEchor daemon supports running containers with CDI devices if the requested
...
`--cdi-spec-dir` flag for the `iechord` CLI.
...
directories using the `iechor info` command.
...
iEchor supports soft links for the iEchor data directory (`/var/lib/iechor`) and
for `/var/lib/iechor/tmp`. The `IECHOR_TMPDIR` and the data directory can be
set like this:

```console
$ export IECHOR_TMPDIR=/mnt/disk2/tmp
$ sudo -E iechord --data-root /var/lib/iechor -H unix://
````

#### Default cgroup parent

The `--cgroup-parent` option lets you set the default cgroup parent
for containers. If this option isn't set, it defaults to `/iechor` for
...
`/sys/fs/cgroup/memory/user.slice/user-a.slice/user-a-b.slice/iechor-<id>.scope`.

This setting can also be set per container, using the `--cgroup-parent`
option on `iechor create` and `iechor run`, and takes precedence over
...
Port `9323` is the [default port associated with iEchor
...
using flags. The iEchor daemon fails to start if an option is duplicated between
...
starting the iEchor daemon. A non-zero exit code is returned for invalid
configuration files.

```console
$ iechord --validate --config-file=/tmp/valid-config.json
configuration OK

$ echo $?
0

$ iechord --validate --config-file /tmp/invalid-config.json
unable to configure the iEchor daemon with file /tmp/invalid-config.json: the following directives don't match any configuration option: unknown-option
...
`/etc/iechor/daemon.json`. Use the `--config-file` flag to specify a
...
```json
{
  ...
  "containerd-namespace": "iechor",
  "containerd-plugins-namespace": "iechor-plugins",
  "data-root": "",
  "debug": true,
  ...
  "icc": false,
  "init": false,
  "init-path": "/usr/libexec/iechor-init",
  "insecure-registries": [],
  "ip": "0.0.0.0",
  "ip-forward": false,
  "ip-masq": false,
  ...
  "userland-proxy-path": "/usr/libexec/iechor-proxy",
  "userns-remap": ""
}
```
...
`%programdata%\iechor\config\daemon.json`. Use the `--config-file` flag
...
The `default-runtime` option is by default unset, in which case iechord automatically detects the runtime.
This detection is based on if the `containerd` flag is set.

Accepted values:

- `com.iechor.hcsshim.v1` - This is the built-in runtime that iEchor has used since Windows supported was first added and uses the v1 HCS API's in Windows.
...
and a global event in Windows with the key `Global\iechor-daemon-config-$PID`.
...
This section describes how to run multiple iEchor daemons on a single host. To
...
--exec-root=/var/run/iechor            Root of the iEchor execdriver
--data-root=/var/lib/iechor            Root of persisted iEchor data
-p, --pidfile=/var/run/iechor.pid      Path to use for daemon PID file
-H, --host=[]                          Daemon socket(s) to connect to
--iptables=true                        Enable addition of iptables rules
--config-file=/etc/iechor/daemon.json  Daemon configuration file
--tlscacert="~/.iechor/ca.pem"         Trust certs signed only by this CA
--tlscert="~/.iechor/cert.pem"         Path to TLS certificate file
--tlskey="~/.iechor/key.pem"           Path to TLS key file
```
...
- The `-b, --bridge=` flag is set to `iechor0` as default bridge network.
  It is created automatically when you install iEchor.
...
- `--exec-root` is the path where the container state is stored.
  The default value is `/var/run/iechor`.
  Specify the path for your running daemon here.
- `--data-root` is the path where persisted data such as images, volumes, and
  cluster state are stored. The default value is `/var/lib/iechor`. To avoid any
  conflict with other daemons, set this parameter separately for each daemon.
- `-p, --pidfile=/var/run/iechor.pid` is the path where the process ID of the daemon is stored.
  Specify the path for your PID file here.
- `--host=[]` specifies where the iEchor daemon listens for client connections.
  If unspecified, it defaults to `/var/run/iechor.sock`.
- `--iptables=false` prevents the iEchor daemon from adding iptables rules. If
  ...
  iptables rules to expose container ports. If you prevent iEchor from adding
  iptables rules, iEchor also doesn't add IP masquerading rules, even if you set
  `--ip-masq` to `true`. Without IP masquerading rules, iEchor containers can't
  connect to external hosts or the internet when using network other than default bridge.
- `--config-file=/etc/iechor/daemon.json` is the path where configuration file is stored.
  You can use it instead of daemon flags. Specify the path for each daemon.
- `--tls*` iEchor daemon supports `--tlsverify` mode that enforces encrypted and authenticated remote connections.
  The `--tls*` options enable use of specific certificates for individual daemons.

Example script for a separate “bootstrap” instance of the iEchor daemon without network:

```console
$ sudo iechord \
        -H unix:///var/run/iechor-bootstrap.sock \
        -p /var/run/iechor-bootstrap.pid \
        --iptables=false \
        --ip-masq=false \
        --bridge=none \
        --data-root=/var/lib/iechor-bootstrap \
        --exec-root=/var/run/iechor-bootstrap
```
...
```json
{
  "default-network-opts": {
    "bridge": {
      "com.iechor.network.bridge.host_binding_ipv4": "127.0.0.1",
      "com.iechor.network.driver.mtu": "1234"
    }
  }
}
```
...
```console
$ iechor network create mynet
$ iechor network inspect mynet --format "{{json .Options}}"
{"com.iechor.network.bridge.host_binding_ipv4":"127.0.0.1","com.iechor.network.driver.mtu":"1234"}
```
...
```console
$ sudo iechord \
  --default-network-opt bridge=com.iechor.network.bridge.host_binding_ipv4=127.0.0.1 \
  --default-network-opt bridge=com.iechor.network.driver.mtu=1234
```
