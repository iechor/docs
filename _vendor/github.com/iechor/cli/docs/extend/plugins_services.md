...
# Using Volume and Network plugins in iEchor services
...
2. Join swarm, install plugin and create volume on worker. In node 2:
    ...
    ```console
    $ iechor volume create -d tiborvass/sample-volume-plugin --name pluginVol
    ```

3. Create a service using the plugin and volume. In node1:

    ```console
    $ iechor service create --name my-service --mount type=volume,volume-driver=tiborvass/sample-volume-plugin,source=pluginVol,destination=/tmp busybox top

    $ iechor service ls
    z1sj8bb8jnfn  my-service   replicated  1/1       busybox:latest
    ```

    `iechor service ls` shows service 1 instance of service running.

4. Observe the task getting scheduled in node 2:

    ```console
    $ iechor ps --format '{{.ID}}\t {{.Status}} {{.Names}} {{.Command}}'
...
1. Install a global scoped network plugin on both manager and worker. On node1
   and node2:
    ...
    ```console
    $ iechor plugin install bboreham/weave2
...
2. Create a network using plugin on manager. On node1:
    ...
    ```console
    $ iechor network create --driver=bboreham/weave2:latest globalnet
...
containers get scheduled on both manager and worker.
    ...
    ```console
    $ iechor service create --network globalnet --name myservice --replicas=8 mrjana/simpleweb simpleweb
...
4. Scale down the number of instances. On node1:
    ...
    ```console
    $ iechor service scale myservice=0
...
5. Disable and uninstall the plugin on the worker. On node2:
    ...
    ```console
    $ iechor plugin rm -f bboreham/weave2
    bboreham/weave2
    ```
...
scheduled on the master and not on the worker, because the plugin is not available on the worker anymore.
    ...
    ```console
    $ iechor service scale myservice=8
