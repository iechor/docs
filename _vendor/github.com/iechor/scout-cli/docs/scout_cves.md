...
```
iechor scout cves [OPTIONS] [IMAGE|DIRECTORY|ARCHIVE]
```
...
| `--multi-stage`        |               |            | Show packages from multi-stage iEchor builds                                                                                                                                                                                                                                                                          |
...
| `--only-stage`         | `stringSlice` |            | Comma separated list of multi-stage iEchor build stage names                                                                                                                                                                                                                                                          |
...
The `iechor scout cves` command analyzes a software artifact for vulnerabilities.
...
### Display vulnerabilities from a `iechor save` tarball

```console
$ iechor save alpine > alpine.tar

$ iechor scout cves archive://alpine.tar
...
```console
$ skopeo copy --override-os linux iechor://alpine oci:alpine

$ iechor scout cves oci-dir://alpine
...
```console
$ iechor scout cves fs://.
```

### Export vulnerabilities to a SARIF JSON file

```console
$ iechor scout cves --format sarif --output alpine.sarif.json alpine
Analyzing image alpine
✓ SBOM of image already cached, 18 packages indexed
✓ No vulnerable package detected
...
```console
$ iechor scout cves --format markdown alpine
✓ Pulled
...
```console
$ iechor scout cves --format only-packages --only-package-type golang --only-vuln-packages golang:1.18.0
✓ Pulled
...
scores to the `iechor scout cves` output. EPSS scores are estimates of the likelihood (probability)
...
...
 ...
 ✗ HIGH CVE-2023-52425
   https://scout.iechor.com/v/CVE-2023-52425
...
of `iechor scout cves` based on these scores. For example,
to only show vulnerabilities with an EPSS score higher than 0.5:

```console
$ iechor scout cves --epss --epss-score 0.5 nginx
...
...
 ...
 ✗ LOW CVE-2023-44487
   https://scout.iechor.com/v/CVE-2023-44487
...
```console
$ iechor scout cves --epss --epss-date 2024-01-02 nginx
```
...
```console
$ syft -o spdx-json alpine:3.16.1 | iechor scout cves sbom://
 ✔ Pulled image
...
pkg:apk/alpine/zlib@1.2.12-r1?arch=aarch64&distro=alpine-3.16.1
    ...
    ✗ CRITICAL CVE-2022-37434
      https://scout.iechor.com/v/CVE-2022-37434
